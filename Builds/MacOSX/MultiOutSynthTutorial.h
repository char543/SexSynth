...
public:
    ...
    void processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages) override
    {
        ...
        midiMessages.clear(); // Clear incoming MIDI messages for the current block
        synth.renderNextBlock(buffer, midiMessages, 0, buffer.getNumSamples()); // Render audio output

        // Other processing (effects, etc.) can be added here
    }
